...
class MainWindow(QMainWindow):
    ...
    def is_valid_youtube_url(self, url):
        """验证YouTube URL是否有效"""
        patterns = [
            r'https?://(www\.)?youtube\.com/@?[\w-]+(/shorts)?/?$',
            r'https?://(www\.)?youtube\.com/@?[\w-]+(/videos)?/?$',
            r'https?://(www\.)?youtube\.com/@?[\w-]+(/featured)?/?$',
            r'https?://(www\.)?youtube\.com/channel/[\w-]+(/shorts)?/?$',
            r'https?://(www\.)?youtube\.com/c/[\w-]+(/shorts)?/?$',
            r'https?://(www\.)?youtube\.com/user/[\w-]+(/shorts)?/?$'
        ]
    ...
    def get_channel_name_from_url(self, url):
        ...
        base_url = re.sub(r'/shorts$|/videos$|/featured$', '', url)
        match = re.search(r'youtube\.com/@?([\w-]+)', base_url)
        if match:
